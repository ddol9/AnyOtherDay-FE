...
export const guardianAPI = {
  ...
  async login(email: string, password: string) {
    ...
    const response = await fetch(
      `${BASE_URL}/guardians/login?${params.toString()}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const reportAPI = {
  // AI 레포트 생성
  async createReport(data: { recordId: number; analysisResult: string }) {
  ...
  async getRecentReports(limit: number = 10) {
    const response = await fetch(`${BASE_URL}/reports?limit=${limit}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
